...
const MermaidPlugin = (md, options) => {
  ...
  md.renderer.rules.fence = (tokens, idx, ops, env, slf) => {
    ...
    if (token.info === "mermaid") {
      ...
      return `<div class="${config.prefix}-mermaid" ${line !== void 0 ? "data-line=" + line : ""} data-mermaid-theme=${options.themeRef.value} data-code="${code}">${code}</div>`;
    }
...
const compareHtml = (newHtml, currentHtml) => {
  const newNodes = splitHtml(newHtml);
  const currentNodes = splitHtml(currentHtml);
  const updates = [];
  const deletes = [];
  newNodes.forEach((node, index2) => {
    if (!currentNodes[index2]) {
      updates.push({
        index: index2,
        newNode: node.outerHTML
      });
    } else if (currentNodes[index2].outerHTML !== node.outerHTML) {
      updates.push({
        index: index2,
        newNode: node.outerHTML
      });
...
const UpdateOnDemand = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(props) {
    ...
    const firstHtml = vue.ref(props.html);
    const updateHtmlContent = (updates, deletes) => {
      if (!htmlContainer.value) return;
      deletes.reverse().forEach((index2) => {
        var _a, _b;
        (_b = (_a = htmlContainer.value) == null ? void 0 : _a.children[index2]) == null ? void 0 : _b.remove();
      });
      updates.forEach(({
        index: index2,
        newNode
        ...
        var _a, _b;
        const targetNode = (_a = htmlContainer.value) == null ? void 0 : _a.children[index2];
        if (targetNode) {
          targetNode.outerHTML = newNode;
        } else {
          (_b = htmlContainer.value) == null ? void 0 : _b.insertAdjacentHTML("beforeend", newNode);
        }
      });
    };
    vue.watch(() => props.html, (newHtml, oldHtml) => {
      const {
        updates,
        deletes
      } = compareHtml(newHtml, oldHtml || "");
