...
const useMarkdownIt = (props, previewOnly) => {
  ...
  const updatedTodo = () => {
    ...
    vue.nextTick(() => {
      replaceMermaid().then(() => {
        var _a;
        dom.zoomMermaid((_a = rootRef.value) == null ? void 0 : _a.querySelectorAll(`#${editorId} .${config.prefix}-mermaid`));
      });
    });
  };
  const markHtml = () => {
      ...
      if (props.setting.preview) {
        vue.nextTick(() => {
          replaceMermaid().then(() => {
            var _a;
            dom.zoomMermaid(
              (_a = rootRef.value) == null ? void 0 : _a.querySelectorAll(`#${editorId} .${config.prefix}-mermaid`)
            );
          });
          eventName.bus.emit(editorId, eventName.CATALOG_CHANGED, headsRef.value);
        });
